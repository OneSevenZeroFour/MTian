...
import { Layout, Menu, Breadcrumb, Dropdown,Input,Select,DatePicker,Spin} from 'antd';
...
class Xtentment extends React.Component{
		...
		componentWillMount(){
			var self = this;
			var kws=this.state.citys
			var kw=encodeURI(kws)
			var urlss='http://120.76.205.241:8000/house/maitianzaixian?cityid=0&kw='+kw+'&type=3&pageToken=1&apikey=RMw9hk7rtVbPfEkQZduqrCvuWs9PSeU7ePDQP0YKrtGQKxuG5d7uiWYeJ4t8vkPF'
				axios.get('http://localhost:23456/cina',
			    	{ params: {
	      					para:urlss
	    				}
		...
		render(){
			return(
				<div className="housejw">
					...
					<div className="jwconent">{this.state.data && this.state.data.map((item,idx)=>{
						return  <li key={idx} className="clearfix conten" >
								<div className="pri fl">
                                    <Link target="_blank"  to="/xrentlist">
